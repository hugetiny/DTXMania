...
namespace DTXMania
{
	internal class CPluginHost : IPluginHost
	{
		...
		{
			get { return CDTXMania.app.TextureFormat; }
		}
		public CTimer Timer
		{
			get { return CDTXMania.app.Timer; }
		}
		public CSound管理 Sound管理
		{
			get { return CDTXMania.app.Sound管理; }
		}
		...
		{
			get { return ( CDTXMania.app.r現在のステージ != null ) ? CDTXMania.app.r現在のステージ.eステージID : CStage.Eステージ.何もしない; }
		}
		public CStage.Eフェーズ e現在のフェーズ
		{
			get { return ( CDTXMania.app.r現在のステージ != null ) ? CDTXMania.app.r現在のステージ.eフェーズID : CStage.Eフェーズ.共通_通常状態; }
		}
		public bool t入力を占有する(IPluginActivity act)
		{
			...
			CDTXMania.app.act現在入力を占有中のプラグイン = act;
			return true;
		}
		public bool t入力の占有を解除する(IPluginActivity act)
		{
			if (CDTXMania.app.act現在入力を占有中のプラグイン == null || CDTXMania.app.act現在入力を占有中のプラグイン != act)
				return false;
		...
		{
			if( CDTXMania.app.Skin != null )
				CDTXMania.app.Skin[ sound ].t再生する();
		}
