...
namespace DTXMania
{
	public class CConfigIni
	{
		...
		{
			//bool ts = CDTXMania.Instance.ConfigIni.bAutoPlay.Bass;			// #24415 2011.2.21 yyagi: FLIP時のリザルトにAUTOの記録が混ざらないよう、AUTOのフラグもswapする
			//CDTXMania.Instance.ConfigIni.bAutoPlay.Bass = CDTXMania.Instance.ConfigIni.bAutoPlay.Guitar;
			//CDTXMania.Instance.ConfigIni.bAutoPlay.Guitar = ts;
			...
			{
				bool b = CDTXMania.Instance.ConfigIni.bAutoPlay[i + (int)Eレーン.BsR];
				CDTXMania.Instance.ConfigIni.bAutoPlay[i + (int)Eレーン.BsR] = CDTXMania.Instance.ConfigIni.bAutoPlay[i + (int)Eレーン.GtR];
				CDTXMania.Instance.ConfigIni.bAutoPlay[i + (int)Eレーン.GtR] = b;
			}

			CDTXMania.Instance.ConfigIni.bIsSwappedGuitarBass_AutoFlagsAreSwapped = !CDTXMania.Instance.ConfigIni.bIsSwappedGuitarBass_AutoFlagsAreSwapped;
		}
		public void SwapGuitarBassInfos_PlaySettings()			// #35417 2015.8.18 yyagi: 演奏設定のFLIP機能を追加
		{
			...
			CDTXMania.Instance.ConfigIni.bIsSwappedGuitarBass_PlaySettingsAreSwapped = !CDTXMania.Instance.ConfigIni.bIsSwappedGuitarBass_PlaySettingsAreSwapped;
	...
	//------------------------------
			...
			Uri uriRoot = new Uri(System.IO.Path.Combine(CDTXMania.Instance.strEXEのあるフォルダ, "System" + System.IO.Path.DirectorySeparatorChar));
			if (strSystemSkinSubfolderFullName != null && strSystemSkinSubfolderFullName.Length == 0)
			{
				...
				strSystemSkinSubfolderFullName = System.IO.Path.Combine(CDTXMania.Instance.strEXEのあるフォルダ, "System" + System.IO.Path.DirectorySeparatorChar + "Default" + System.IO.Path.DirectorySeparatorChar);
			}
									...
									case Eセクション種別.System:
										...
										//----------------------------------------
											...
											else if (str3.Equals("SkinPath"))
											{
												...
												{
													absSkinPath = System.IO.Path.Combine(CDTXMania.Instance.strEXEのあるフォルダ, "System");
