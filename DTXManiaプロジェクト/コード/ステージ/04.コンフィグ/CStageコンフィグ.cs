	...
	internal class CStageコンフィグ : CStage
	{
		...
		public override int On進行描画()
		{
			if (base.b活性化してる)
			{
				...
				{
					if (CDTXMania.Instance.Input管理.Keyboard.bキーが押された((int)SharpDX.DirectInput.Key.Escape) || CDTXMania.Instance.Pad.bCancelPadIsPressedDGB())
					{
					...
					#region [ ← ]
					else if (CDTXMania.Instance.Input管理.Keyboard.bキーが押された((int)SharpDX.DirectInput.Key.Left))   // 左カーソルキー
					{
						if (!this.bメニューにフォーカス中)
						{
							if (this.eItemPanelモード == EItemPanelモード.キーコード一覧)
					...
					#region [ → ]
					else if (CDTXMania.Instance.Input管理.Keyboard.bキーが押された((int)SharpDX.DirectInput.Key.Right))  // 右カーソルキー
					{
						#region [ EXIT ]
						if (this.n現在のメニュー番号 == 4)
						{
