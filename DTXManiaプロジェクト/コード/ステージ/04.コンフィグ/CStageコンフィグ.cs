	...
	internal class CStageコンフィグ : CStage
	{
		...
		public override void On活性化()
		{
			...
			try
			{
				...
				fDisplayLagTimeBaseMs.Drums = (float) CDTXMania.Instance.ConfigIni.nJudgeLinePosOffset.Drums / ( CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Drums + 1 );
				fDisplayLagTimeBaseMs.Guitar = (float) CDTXMania.Instance.ConfigIni.nJudgeLinePosOffset.Guitar / ( CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Guitar + 1 );
				fDisplayLagTimeBaseMs.Bass = (float) CDTXMania.Instance.ConfigIni.nJudgeLinePosOffset.Bass / ( CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Bass + 1 );
			}
			finally
			{
		...
		public override void On非活性化()
		{
			...
			try
			{
				CDTXMania.Instance.ConfigIni.t書き出し( CDTXMania.Instance.strEXEのあるフォルダ + "Config.ini" );	// CONFIGだけ
				...
				CDTXMania.Instance.ConfigIni.nJudgeLinePosOffset.Drums =  (int) ( fDisplayLagTimeBaseMs.Drums  * ( CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Drums + 1 ) );
				CDTXMania.Instance.ConfigIni.nJudgeLinePosOffset.Guitar = (int) ( fDisplayLagTimeBaseMs.Guitar * ( CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Guitar + 1 ) );
				CDTXMania.Instance.ConfigIni.nJudgeLinePosOffset.Bass =   (int) ( fDisplayLagTimeBaseMs.Bass   * ( CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Bass + 1 ) );
		...
		public override int On進行描画()
		{
			...
			if ( !CDTXMania.Instance.EnumSongs.IsEnumerating || CDTXMania.Instance.actEnumSongs.bコマンドでの曲データ取得 != true )
			{
				if ( ( CDTXMania.Instance.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.Escape ) || CDTXMania.Instance.Pad.b押された( E楽器パート.DRUMS, Eパッド.FT ) ) || CDTXMania.Instance.Pad.b押されたGB( Eパッド.FT ) )
				{
				...
				#region [ ← ]
				else if ( CDTXMania.Instance.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.LeftArrow ) )	// 左カーソルキー
				{
					if ( !this.bメニューにフォーカス中 )
					{
						if ( this.eItemPanelモード == EItemPanelモード.キーコード一覧 )
				...
				#endregion
				else if ( ( CDTXMania.Instance.Pad.b押されたDGB( Eパッド.CY ) || CDTXMania.Instance.Pad.b押された( E楽器パート.DRUMS, Eパッド.RD ) ) || ( CDTXMania.Instance.Pad.b押された( E楽器パート.DRUMS, Eパッド.LC ) || ( CDTXMania.Instance.ConfigIni.bEnterがキー割り当てのどこにも使用されていない && CDTXMania.Instance.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.Return ) ) ) )
				{
					#region [ EXIT ]
				...
				#region [ → ]
				else if ( CDTXMania.Instance.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.RightArrow ) )	// 右カーソルキー
				{
					#region [ EXIT ]
					if ( this.n現在のメニュー番号 == 4 )
					{
