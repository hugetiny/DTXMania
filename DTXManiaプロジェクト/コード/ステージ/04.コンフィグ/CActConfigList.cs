...
namespace DTXMania
{
	internal class CActConfigList : CActivity
	{
		...
		public void t項目リストの設定_System()
		{
			...
			this.iCommonDark = new CItemList("Dark", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eDark,
			...
			this.iSystemRisky = new CItemInteger("Risky", 0, 10, CDTXMania.Instance.ConfigIni.nRisky,
				"Riskyモードの設定:\n" +
			...
			int nDGmode = (CDTXMania.Instance.ConfigIni.bGuitar有効 ? 2 : 0) + (CDTXMania.Instance.ConfigIni.bDrums有効 ? 1 : 0) - 1;
			this.iSystemGRmode = new CItemList("Drums & GR ", CItemBase.Eパネル種別.通常, nDGmode,
				...
				"Both: Both Drums and Guitar/Bass are available.",
				new string[] { "DrOnly", "GROnly", "Both" });
			this.list項目リスト.Add(this.iSystemGRmode);

			this.iCommonPlaySpeed = new CItemInteger("PlaySpeed", 5, 40, CDTXMania.Instance.ConfigIni.n演奏速度,
				"曲の演奏速度を、速くしたり遅くしたりすることができます。\n" +
				"（※一部のサウンドカードでは正しく再生できない可能性があります。）\n" +
				"\n" +
				"TimeStretchがONのときに、演奏速度をx0.850以下にすると、チップのズレが大きくなります。",
			...
			this.iSystemTimeStretch = new CItemToggle("TimeStretch", CDTXMania.Instance.ConfigIni.bTimeStretch,
				"演奏速度の変更方式:\n" +
				"ONにすると、演奏速度の変更を、周波数変更ではなくタイムストレッチで行います。\n" +
				"\n" +
				"タイムストレッチ使用時は、サウンド処理により多くのCPU性能を使用します。\n" +
				"また、演奏速度をx0.850以下にすると、チップのズレが大きくなります。",
			...
			this.iSystemFullscreen = new CItemToggle("Fullscreen", CDTXMania.Instance.ConfigIni.b全画面モード,
				"画面モード設定：\n" +
				"ON で全画面モード、OFF でウィンドウモードになります。",
				"Fullscreen mode or window mode.");
			this.list項目リスト.Add(this.iSystemFullscreen);
			this.iSystemStageFailed = new CItemToggle("StageFailed", CDTXMania.Instance.ConfigIni.bSTAGEFAILED有効,
				"STAGE FAILED 有効：\n" +
				"ON にすると、ゲージがなくなった時に STAGE FAILED となり演奏が中断されます。\n" +
				"OFF の場合は、ゲージがなくなっても最後まで演奏できます。",
				"Turn OFF if you don't want to encount GAME OVER.");
			this.list項目リスト.Add(this.iSystemStageFailed);
			this.iSystemRandomFromSubBox = new CItemToggle("RandSubBox", CDTXMania.Instance.ConfigIni.bランダムセレクトで子BOXを検索対象とする,
				"子BOXをRANDOMの対象とする：\n" +
				"ON にすると、RANDOM SELECT 時に子BOXも選択対象とします。",
				"Turn ON to use child BOX (subfolders) at RANDOM SELECT.");
			this.list項目リスト.Add(this.iSystemRandomFromSubBox);


			this.iSystemAdjustWaves = new CItemToggle("AdjustWaves", CDTXMania.Instance.ConfigIni.bWave再生位置自動調整機能有効,
				"サウンド再生位置自動補正：\n" +
				"ハードウェアやOSに起因するサウンドのずれを強制的に補正します。\n" +
				"BGM のように再生時間の長い音声データが使用されている曲で効果があります。" +
				"\n" +
			...
			this.iSystemVSyncWait = new CItemToggle("VSyncWait", CDTXMania.Instance.ConfigIni.b垂直帰線待ちを行う,
				"垂直帰線同期：\n" +
				"画面の描画をディスプレイの垂直帰線中に行なう場合には ON を指定します。\n" +
				"ON にすると、ガタつきのない滑らかな画面描画が実現されます。",
			...
			this.iSystemAVI = new CItemToggle("AVI", CDTXMania.Instance.ConfigIni.bAVI有効,
				"AVIの使用：\n" +
				"演奏中に動画(AVI)を再生する場合にON にします。\n" +
				"AVI の再生には、それなりのマシンパワーが必要とされます。",
				"To use AVI playback or not.");
			this.list項目リスト.Add(this.iSystemAVI);
			this.iSystemForceAVIFullscreen = new CItemToggle("FullAVI", CDTXMania.Instance.ConfigIni.bForceAVIFullscreen,
				"旧AVIの全画面表示：\n" +
			...
			this.iSystemBGA = new CItemToggle("BGA", CDTXMania.Instance.ConfigIni.bBGA有効,
				"BGAの使用：\n" +
			...
			this.iSystemPreviewSoundWait = new CItemInteger("PreSoundWait", 0, 0x2710, CDTXMania.Instance.ConfigIni.n曲が選択されてからプレビュー音が鳴るまでのウェイトms,
				"プレビュー音演奏までの時間：\n" +
			...
			this.iSystemPreviewImageWait = new CItemInteger("PreImageWait", 0, 0x2710, CDTXMania.Instance.ConfigIni.n曲が選択されてからプレビュー画像が表示開始されるまでのウェイトms,
				"プレビュー画像表示までの時間：\n" +
			...
			this.iSystemDebugInfo = new CItemToggle("Debug Info", CDTXMania.Instance.ConfigIni.b演奏情報を表示する,
				"演奏情報の表示：\n" +
				"演奏中、BGA領域の下部に演奏情報（FPS、BPM、演奏時間など）を表示します。\n" +
				"また、小節線の横に小節番号が表示されるようになります。",
			...
			this.iSystemBGAlpha = new CItemInteger("BG Alpha", 0, 0xff, CDTXMania.Instance.ConfigIni.n背景の透過度,
				"背景画像の透明割合：\n" +
			...
			this.iSystemBGMSound = new CItemToggle("BGM Sound", CDTXMania.Instance.ConfigIni.bBGM音を発声する,
				"BGMの再生：\n" +
				"これをOFFにすると、BGM を再生しなくなります。",
				"Turn OFF if you don't want to play BGM.");
			this.list項目リスト.Add(this.iSystemBGMSound);
			this.iSystemAudienceSound = new CItemToggle("Audience", CDTXMania.Instance.ConfigIni.b歓声を発声する,
				"歓声の再生：\n" +
			...
			this.iSystemDamageLevel = new CItemList("DamageLevel", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eダメージレベル,
				"ゲージ減少割合：\n" +
			...
			this.iSystemSaveScore = new CItemToggle("SaveScore", CDTXMania.Instance.ConfigIni.bScoreIniを出力する,
				"演奏記録の保存：\n" +
				"これを ON にすると、演奏記録を ～.score.ini ファイルに保存します。",
			...
			this.iSystemChipVolume = new CItemInteger("ChipVolume", 0, 100, CDTXMania.Instance.ConfigIni.n手動再生音量,
				"打音の音量：\n" +
			...
			this.iSystemAutoChipVolume = new CItemInteger("AutoVolume", 0, 100, CDTXMania.Instance.ConfigIni.n自動再生音量,
				"自動再生音の音量：\n" +
			...
			this.iSystemStoicMode = new CItemToggle("StoicMode", CDTXMania.Instance.ConfigIni.bストイックモード,
				"ストイック（禁欲）モード：\n" +
				"以下をまとめて表示ON/OFFします。\n" +
				"・プレビュー画像/動画\n" +
				"・リザルト画像/動画\n" +
			...
			this.iSystemShowLag = new CItemList("ShowLagTime", CItemBase.Eパネル種別.通常, CDTXMania.Instance.ConfigIni.nShowLagType,
				"ズレ時間表示：\n" +
			...
			this.iSystemAutoResultCapture = new CItemToggle("Autosaveresult", CDTXMania.Instance.ConfigIni.bIsAutoResultCapture,
				"リザルト画像自動保存機能：\n" +
				"ONにすると、ハイスコア/ハイスキル時に、自動的にリザルト画像を曲データと同じフォルダに保存します。",
				"AutoSaveResult:\n" +
			...
			this.iSystemJudgeDispPriority = new CItemList("JudgePriority", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.e判定表示優先度,
			...
			this.iSystemBufferedInput = new CItemToggle("BufferedInput", CDTXMania.Instance.ConfigIni.bバッファ入力を行う,
				"バッファ入力モード：\n" +
			...
			this.iLogOutputLog = new CItemToggle("TraceLog", CDTXMania.Instance.ConfigIni.bログ出力,
				"Traceログ出力：\n" +
			...
			this.iSystemLoadsoundspeed = new CItemToggle("LoadSoundSpeed", CDTXMania.Instance.ConfigIni.b曲読み込みを高速化する,
				"曲読み込みを高速化する：\n" +
			...
			this.iSystemSoundType = new CItemList("SoundType", CItemList.Eパネル種別.通常, CDTXMania.Instance.ConfigIni.nSoundDeviceType,
				"サウンドの出力方式:\n" +
			...
			this.iSystemWASAPIBufferSizeMs = new CItemInteger("WASAPI BufferSize", 0, 99999, CDTXMania.Instance.ConfigIni.nWASAPIBufferSizeMs,
					"WASAPI使用時のバッファサイズ:\n" +
				"\n" +
				//  "0～99999ms を指定可能です。\n" +
			...
			this.iSystemASIODevice = new CItemList("ASIO device", CItemList.Eパネル種別.通常, CDTXMania.Instance.ConfigIni.nASIODevice,
				"ASIOデバイス:\n" +
			...
			// #24820 2013.1.3 yyagi
			//this.iSystemASIOBufferSizeMs = new CItemInteger("ASIOBuffSize", 0, 99999, CDTXMania.Instance.ConfigIni.nASIOBufferSizeMs,
			//    "ASIO使用時のバッファサイズ:\n" +
			//    "0～99999ms を指定可能です。\n" +
			//    "推奨値は0で、サウンドデバイスでの\n" +
			//    "設定値をそのまま使用します。\n" +
			...
			this.iSystemSoundTimerType = new CItemToggle("UseOSTimer", CDTXMania.Instance.ConfigIni.bUseOSTimer,
				"OSタイマーを使用するかどうか:\n" +
			...
			this.iSystemMasterVolume = new CItemInteger("MasterVolume", 0, 100, CDTXMania.Instance.ConfigIni.nMasterVolume,
				"マスターボリュームの設定:\n" +
				"全体の音量を設定します。\n" +
				"0が無音で、100が最大値です。\n" +
				"(WASAPI/ASIO時のみ有効です)",
			...
			this.iSystemUseBoxDefSkin = new CItemToggle("Skin (Box)", CDTXMania.Instance.ConfigIni.bUseBoxDefSkin,
				"Music boxスキンの利用：\n" +
		...
		public void t項目リストの設定_Drums()
		{
			...
			this.iDrumsLeftCymbal = new CItemToggle("    LeftCymbal", CDTXMania.Instance.ConfigIni.bAutoPlay.LC,
				"左シンバルを自動で演奏します。",
				"To play LeftCymbal automatically.");
			this.list項目リスト.Add(this.iDrumsLeftCymbal);

			this.iDrumsHiHat = new CItemToggle("    HiHat", CDTXMania.Instance.ConfigIni.bAutoPlay.HH,
				"ハイハットを自動で演奏します。\n" +
				"（クローズ、オープンとも）",
			...
			this.iDrumsSnare = new CItemToggle("    Snare", CDTXMania.Instance.ConfigIni.bAutoPlay.SD,
				"スネアを自動で演奏します。",
				"To play Snare automatically.");
			this.list項目リスト.Add(this.iDrumsSnare);

			this.iDrumsBass = new CItemToggle("    BassDrum", CDTXMania.Instance.ConfigIni.bAutoPlay.BD,
				"バスドラムを自動で演奏します。",
				"To play Bass Drum automatically.");
			this.list項目リスト.Add(this.iDrumsBass);

			this.iDrumsHighTom = new CItemToggle("    HighTom", CDTXMania.Instance.ConfigIni.bAutoPlay.HT,
				"ハイタムを自動で演奏します。",
				"To play High Tom automatically.");
			this.list項目リスト.Add(this.iDrumsHighTom);

			this.iDrumsLowTom = new CItemToggle("    LowTom", CDTXMania.Instance.ConfigIni.bAutoPlay.LT,
				"ロータムを自動で演奏します。",
				"To play Low Tom automatically.");
			this.list項目リスト.Add(this.iDrumsLowTom);

			this.iDrumsFloorTom = new CItemToggle("    FloorTom", CDTXMania.Instance.ConfigIni.bAutoPlay.FT,
				"フロアタムを自動で演奏します。",
				"To play Floor Tom automatically.");
			this.list項目リスト.Add(this.iDrumsFloorTom);

			this.iDrumsCymbalRide = new CItemToggle("    Cym/Ride", CDTXMania.Instance.ConfigIni.bAutoPlay.CY,
			...
			this.iDrumsScrollSpeed = new CItemInteger("ScrollSpeed", 0, 0x7cf, CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Drums,
			...
			#endregion
			//this.iDrumsSudden = new CItemToggle( "Sudden", CDTXMania.Instance.ConfigIni.bSudden.Drums,
			//    "ドラムチップが譜面の下の方から表\n" +
			//    "示されるようになります。",
			...
			//this.iDrumsHidden = new CItemToggle( "Hidden", CDTXMania.Instance.ConfigIni.bHidden.Drums,
			//    "ドラムチップが譜面の下の方で表示\n" +
			//    "されなくなります。",
			...
			//this.iDrumsInvisible = new CItemList( "Invisible", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.eInvisible.Drums,
			//    "ドラムチップの透明化\n" +
			//    "OFF: チップを常に表示します。\n" +
			//    "SEMI: 通常はチップを透明にしますが、\n" +
			...
			//this.iDrumsLanePosition = new CItemList( "LanePosition", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.eドラムレーン表示位置,
			//    "ドラムのレーン表示位置を設定します。\n" +
			...
			this.iDrumsReverse = new CItemToggle("Reverse", CDTXMania.Instance.ConfigIni.bReverse.Drums,
			...
			this.iDrumsTight = new CItemToggle("Tight", CDTXMania.Instance.ConfigIni.bTight,
			...
			this.iDrumsComboPosition = new CItemList("ComboPosition", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.ドラムコンボ文字の表示位置,
			...
			#endregion
			#region [ Position ]
			this.iDrumsPosition = new CItemList("Position", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.判定文字表示位置.Drums,
				"ドラムの判定文字の表示位置を指定\n" +
				"します。\n" +
				"  P-A: レーン上\n" +
				"  P-B: 判定ライン下\n" +
			...
			//this.iDrumsJudgeDispPriority = new CItemList( "JudgePriority", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.e判定表示優先度.Drums,
			//    "判定文字列とコンボ表示の優先順位を\n" +
			//    "指定します。\n" +
			//    "\n" +
			//    " Under: チップの下に表示します。\n" +
			...
			#endregion
			#region [ Group ]
			this.iSystemHHGroup = new CItemList("HH Group", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eHHGroup,
				"ハイハットレーン打ち分け設定：\n" +
			...
			this.iSystemFTGroup = new CItemList("FT Group", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eFTGroup,
				"フロアタム打ち分け設定：\n" +
			...
			this.iSystemCYGroup = new CItemList("CY Group", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eCYGroup,
				"シンバルレーン打ち分け設定：\n" +
			...
			this.iSystemBDGroup = new CItemList("BD Group", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eBDGroup,		// #27029 2012.1.4 from
				"バス数設定：\n" +
				...
				"また、BD-1 を選択している間はいくつかのオプションが変更できなくなります。",
				"BD-0: HP|BD\n" +
				" HiHatPedal is HiHat.\n" +
				"BD-1: HP&BD\n" +
				" HiHatPedal is Bass.\n" +
			...
			this.iSystemCymbalFree = new CItemToggle("CymbalFree", CDTXMania.Instance.ConfigIni.bシンバルフリー,
				"シンバルフリーモード：\n" +
			...
			this.iSystemHitSoundPriorityHH = new CItemList("HH Priority", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eHitSoundPriorityHH,
				"発声音決定の優先順位：\n" +
			...
			this.iSystemHitSoundPriorityFT = new CItemList("FT Priority", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eHitSoundPriorityFT,
				"発声音決定の優先順位：\n" +
			...
			this.iSystemHitSoundPriorityCY = new CItemList("CY Priority", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eHitSoundPriorityCY,
				"発声音決定の優先順位：\n" +
			...
			this.iSystemFillIn = new CItemToggle("FillIn", CDTXMania.Instance.ConfigIni.bフィルイン有効,
				"フィルインエフェクトの使用：\n" +
			...
			this.iSystemHitSound = new CItemToggle("HitSound", CDTXMania.Instance.ConfigIni.bドラム打音を発声する,
				"打撃音の再生：\n" +
			...
			this.iSystemSoundMonitorDrums = new CItemToggle("DrumsMonitor", CDTXMania.Instance.ConfigIni.b演奏音を強調する.Drums,
				"ドラム音モニタ：\n" +
			...
			this.iSystemMinComboDrums = new CItemInteger("D-MinCombo", 1, 0x1869f, CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数.Drums,
				"表示可能な最小コンボ数（ドラム）：\n" +
			...
			this.iDrumsInputAdjustTimeMs = new CItemInteger("InputAdjust", -99, 99, CDTXMania.Instance.ConfigIni.nInputAdjustTimeMs.Drums,
		...
		public void t項目リストの設定_Guitar()
		{
			...
			this.iGuitarR = new CItemToggle("    R", CDTXMania.Instance.ConfigIni.bAutoPlay.GtR,
				"Rネックを自動で演奏します。",
				"To play R neck automatically.");
			this.list項目リスト.Add(this.iGuitarR);
			this.iGuitarG = new CItemToggle("    G", CDTXMania.Instance.ConfigIni.bAutoPlay.GtG,
				"Gネックを自動で演奏します。",
				"To play G neck automatically.");
			this.list項目リスト.Add(this.iGuitarG);
			this.iGuitarB = new CItemToggle("    B", CDTXMania.Instance.ConfigIni.bAutoPlay.GtB,
				"Bネックを自動で演奏します。",
				"To play B neck automatically.");
			this.list項目リスト.Add(this.iGuitarB);
			this.iGuitarPick = new CItemToggle("    Pick", CDTXMania.Instance.ConfigIni.bAutoPlay.GtPick,
				"ピックを自動で演奏します。",
				"To play Pick automatically.");
			this.list項目リスト.Add(this.iGuitarPick);
			this.iGuitarW = new CItemToggle("    Wailing", CDTXMania.Instance.ConfigIni.bAutoPlay.GtW,
				"ウェイリングを自動で演奏します。",
				"To play wailing automatically.");
			this.list項目リスト.Add(this.iGuitarW);
			#endregion
			this.iGuitarScrollSpeed = new CItemInteger("ScrollSpeed", 0, 0x7cf, CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Guitar,
			...
			//this.iGuitarSudden = new CItemToggle( "Sudden", CDTXMania.Instance.ConfigIni.bSudden.Guitar,
			//    "ギターチップがヒットバー付近にくる\nまで表示されなくなります。",
			//    "Guitar chips are disappered until they\ncome near the hit bar, and suddenly\nappears." );
			//this.list項目リスト.Add( this.iGuitarSudden );
			//this.iGuitarHidden = new CItemToggle( "Hidden", CDTXMania.Instance.ConfigIni.bHidden.Guitar,
			//    "ギターチップがヒットバー付近で表示\nされなくなります。",
			//    "Guitar chips are hidden by approaching\nto the hit bar. " );
			//this.list項目リスト.Add( this.iGuitarHidden );

			//this.iGuitarInvisible = new CItemList( "Invisible", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.eInvisible.Guitar,
			//    "ギターのチップを全く表示しなくなりま\n" +
			//    "す。暗譜での練習にお使いください。\n" +
			//    "これをONにすると、SuddenとHiddenの\n" +
			//    "効果は無効になります。",
			...
			this.iGuitarReverse = new CItemToggle("Reverse", CDTXMania.Instance.ConfigIni.bReverse.Guitar,
			...
			this.iSystemJudgePosGuitar = new CItemList("JudgePos", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.e判定位置.Guitar,
				"判定ライン表示位置:\n" +
				"判定ラインとRGBボタンが、少し下側に表示されるようになります。",
				"Judge Line position:\n" +
			...
			this.iGuitarPosition = new CItemList("Position", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.判定文字表示位置.Guitar,
				"ギターの判定文字の表示位置を指定します。\n" +
				" P-A: レーン上\n" +
				" P-B: 判定ラインの上\n" +
				" P-C: COMBO の下\n" +
			...
			//this.iGuitarJudgeDispPriority = new CItemList( "JudgePriority", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.e判定表示優先度.Guitar,
			//    "判定文字列とコンボ表示の優先順位を\n" +
			//    "指定します。\n" +
			//    "\n" +
			//    " Under: チップの下に表示します。\n" +
			...
			this.iGuitarRandom = new CItemList("Random", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.eRandom.Guitar,
				"ギターのチップがランダムに降ってきます。\n" +
				"  Part: 小節・レーン単位で交換\n" +
				"  Super: チップ単位で交換\n" +
				"  Hyper: 全部完全に変更",
			...
			this.iGuitarLight = new CItemToggle("Light", CDTXMania.Instance.ConfigIni.bLight.Guitar,
				"ギターチップのないところでピッキングしても BAD になりません。",
				"Even if you pick without any chips, it doesn't become BAD.");
			this.list項目リスト.Add(this.iGuitarLight);
			this.iGuitarLeft = new CItemToggle("Left", CDTXMania.Instance.ConfigIni.bLeft.Guitar,
				"ギターの RGB の並びが左右反転します。（左利きモード）",
				"Lane order 'R-G-B' becomes 'B-G-R' for lefty.");
			this.list項目リスト.Add(this.iGuitarLeft);

			this.iSystemSoundMonitorGuitar = new CItemToggle("GuitarMonitor", CDTXMania.Instance.ConfigIni.b演奏音を強調する.Guitar,
			"ギター音モニタ：\n" +
			"ギター音を他の音より大きめの音量で発声します。\n" +
			"ただし、オートプレイの場合は通常音量で発声されます。",
			"To enhance the guitar chip sound (except autoplay).");
			this.list項目リスト.Add(this.iSystemSoundMonitorGuitar);
			this.iSystemMinComboGuitar = new CItemInteger("G-MinCombo", 1, 0x1869f, CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数.Guitar,
				"表示可能な最小コンボ数（ギター）：\n" +
				"画面に表示されるコンボの最小の数を指定します。\n" +
				"1 ～ 99999 の値が指定可能です。",
			...
			// #23580 2011.1.3 yyagi
			this.iGuitarInputAdjustTimeMs = new CItemInteger("InputAdjust", -99, 99, CDTXMania.Instance.ConfigIni.nInputAdjustTimeMs.Guitar,
		...
		public void t項目リストの設定_Bass()
		{
			...
			this.iBassR = new CItemToggle("    R", CDTXMania.Instance.ConfigIni.bAutoPlay.BsR,
				"Rネックを自動で演奏します。",
				"To play R neck automatically.");
			this.list項目リスト.Add(this.iBassR);
			this.iBassG = new CItemToggle("    G", CDTXMania.Instance.ConfigIni.bAutoPlay.BsG,
				"Gネックを自動で演奏します。",
				"To play G neck automatically.");
			this.list項目リスト.Add(this.iBassG);
			this.iBassB = new CItemToggle("    B", CDTXMania.Instance.ConfigIni.bAutoPlay.BsB,
				"Bネックを自動で演奏します。",
				"To play B neck automatically.");
			this.list項目リスト.Add(this.iBassB);
			this.iBassPick = new CItemToggle("    Pick", CDTXMania.Instance.ConfigIni.bAutoPlay.BsPick,
				"ピックを自動で演奏します。",
				"To play Pick automatically.");
			this.list項目リスト.Add(this.iBassPick);
			this.iBassW = new CItemToggle("    Wailing", CDTXMania.Instance.ConfigIni.bAutoPlay.BsW,
				"ウェイリングを自動で演奏します。",
				"To play wailing automatically.");
			this.list項目リスト.Add(this.iBassW);
			#endregion

			this.iBassScrollSpeed = new CItemInteger("ScrollSpeed", 0, 0x7cf, CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Bass,
			...
			//this.iBassSudden = new CItemToggle( "Sudden", CDTXMania.Instance.ConfigIni.bSudden.Bass,
			//    "ベースチップがヒットバー付近にくる\nまで表示されなくなります。",
			//    "Bass chips are disappered until they\ncome near the hit bar, and suddenly\nappears." );
			//this.list項目リスト.Add( this.iBassSudden );
			//this.iBassHidden = new CItemToggle( "Hidden", CDTXMania.Instance.ConfigIni.bHidden.Bass,
			//    "ベースチップがヒットバー付近で表示\nされなくなります。",
			//    "Bass chips are hidden by approaching\nto the hit bar." );
			//this.list項目リスト.Add( this.iBassHidden );

			//this.iBassInvisible = new CItemList( "InvisibleBlindfold", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.eInvisible.Bass,
			//    "ベースのチップを全く表示しなくなりま\n" +
			//    "す。暗譜での練習にお使いください。\n" +
			//    "これをONにすると、SuddenとHiddenの\n" +
			//    "効果は無効になります。",
			...
			this.iBassReverse = new CItemToggle("Reverse", CDTXMania.Instance.ConfigIni.bReverse.Bass,
			...
			this.iSystemJudgePosBass = new CItemList("JudgePos", CItemBase.Eパネル種別.通常, (int)CDTXMania.Instance.ConfigIni.e判定位置.Bass,
				"判定ライン表示位置:\n" +
				"判定ラインとRGBボタンが、少し下側に表示されるようになります。",
				"Judge Line position:\n" +
				"The judge line and RGB buttons will be displayed lower position.",
			...
			this.iBassPosition = new CItemList("Position", CItemBase.Eパネル種別.通常,
				(int)CDTXMania.Instance.ConfigIni.判定文字表示位置.Bass,
				"ベースの判定文字の表示位置を指定します。\n" +
				" P-A: レーン上\n" +
				" P-B: 判定ラインの上\n" +
				" P-C: COMBO の下\n" +
			...
			//this.iBassJudgeDispPriority = new CItemList( "JudgePriority", CItemBase.Eパネル種別.通常, (int) CDTXMania.Instance.ConfigIni.e判定表示優先度.Bass,
			//"判定文字列とコンボ表示の優先順位を\n" +
			//"指定します。\n" +
			//"\n" +
			//" Under: チップの下に表示します。\n" +
			...
			this.iBassRandom = new CItemList("Random", CItemBase.Eパネル種別.通常,
				(int)CDTXMania.Instance.ConfigIni.eRandom.Bass,
				"ベースのチップがランダムに降ってきます。\n" +
				"  Part: 小節・レーン単位で交換\n" +
				"  Super: チップ単位で交換\n" +
				"  Hyper: 全部完全に変更",
			...
			this.iBassLight = new CItemToggle("Light", CDTXMania.Instance.ConfigIni.bLight.Bass,
				"ベースチップのないところでピッキングしても BAD になりません。",
				"Even if you pick without any chips, it doesn't become BAD.");
			this.list項目リスト.Add(this.iBassLight);
			this.iBassLeft = new CItemToggle("Left", CDTXMania.Instance.ConfigIni.bLeft.Bass,
				"ベースの RGB の並びが左右反転します。（左利きモード）",
				"Lane order 'R-G-B' becomes 'B-G-R' for lefty.");
			this.list項目リスト.Add(this.iBassLeft);

			this.iSystemSoundMonitorBass = new CItemToggle("BassMonitor", CDTXMania.Instance.ConfigIni.b演奏音を強調する.Bass,
			"ベース音モニタ：\n" +
			...
			this.iSystemMinComboBass = new CItemInteger("B-MinCombo", 1, 0x1869f, CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数.Bass,
				"表示可能な最小コンボ数（ベース）：\n" +
				"画面に表示されるコンボの最小の数を指定します。\n" +
				"1 ～ 99999 の値が指定可能です。",
			...
			// #23580 2011.1.3 yyagi
			this.iBassInputAdjustTimeMs = new CItemInteger("InputAdjust", -99, 99, CDTXMania.Instance.ConfigIni.nInputAdjustTimeMs.Bass,
		...
		public void tEnter押下()
		{
			...
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.LC);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsHHC)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.HH);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsHHO)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.HHO);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsSD)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.SD);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsBD)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.BD);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsHT)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.HT);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsLT)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.LT);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsFT)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.FT);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsCY)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.CY);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsRD)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.RD);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignDrumsHP)			// #27029 2012.1.4 from
			{																							//
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.DRUMS, EKeyConfigPad.HP);	//
			}																							//
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarR)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.R);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarG)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.G);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarB)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.B);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarPick)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.Pick);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarWail)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.Wail);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarDecide)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.Decide);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignGuitarCancel)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.GUITAR, EKeyConfigPad.Cancel);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassR)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.R);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassG)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.G);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassB)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.B);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassPick)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.Pick);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassWail)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.Wail);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassDecide)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.Decide);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignBassCancel)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.BASS, EKeyConfigPad.Cancel);
			}
			else if (this.list項目リスト[this.n現在の選択項目] == this.iKeyAssignSystemCapture)
			{
				CDTXMania.Instance.stageコンフィグ.tパッド選択通知(EKeyConfigPart.SYSTEM, EKeyConfigPad.Capture);
			}
			#endregion
			else
			{
				...
				{
					CDTXMania.Instance.ConfigIni.b垂直帰線待ちを行う = this.iSystemVSyncWait.bON;
				...
				else if (this.list項目リスト[this.n現在の選択項目] == this.iSystemBDGroup)					// #27029 2012.1.4 from
				{
					if (this.iSystemBDGroup.n現在選択されている項目番号 == (int)EBDGroup.どっちもBD)
					{
						...
						CDTXMania.Instance.ConfigIni.BackupOf1BD = new CConfigIni.CBackupOf1BD()
						{
					...
					else
					{
						...
						this.iSystemHHGroup.n現在選択されている項目番号 = (int)CDTXMania.Instance.ConfigIni.BackupOf1BD.eHHGroup;
						this.iSystemHitSoundPriorityHH.n現在選択されている項目番号 = (int)CDTXMania.Instance.ConfigIni.BackupOf1BD.eHitSoundPriorityHH;

						CDTXMania.Instance.ConfigIni.BackupOf1BD = null;
					}
				}
				#endregion
		...
		private void tGenerateSkinSample()
		{
			...
			if (nSkinSampleIndex != nSkinIndex)
			{
				...
				{
					TextureFactory.t安全にDisposeする(ref txSkinSample1);
				}
				txSkinSample1 = TextureFactory.tテクスチャの生成(bmDest, false);
				g.Dispose();
				bmDest.Dispose();
				bmSrc.Dispose();
		...
		private void t要素値を上下に変更中の処理()
		{
			...
			{
				CDTXMania.Instance.Sound管理.nMasterVolume = this.iSystemMasterVolume.n現在の値;
			}
		}
		...
		public override void On活性化()
		{
			...
			skinSubFolder_org = CDTXMania.Instance.Skin.GetCurrentSkinSubfolderFullName(true);
		...
		public override void On非活性化()
		{
			...
			base.On非活性化();
			#region [ Skin変更 ]
			if (CDTXMania.Instance.Skin.GetCurrentSkinSubfolderFullName(true) != this.skinSubFolder_org)
			{
				...
				this.iSystemSoundTimerType_initial != this.iSystemSoundTimerType.GetIndex())
			{
				...
				CDTXMania.Instance.Sound管理.t初期化(soundDeviceType,
		...
		public override void OnManagedリソースの解放()
		{
			...
			TextureFactory.tテクスチャの解放(ref this.txSkinSample1);
		...
		public int t進行描画(bool b項目リスト側にフォーカスがある)
		{
			...
			for (int n行番号 = -4; n行番号 < 6; n行番号++)		// n行番号 == 0 がフォーカスされている項目パネル。
			{
				...
				else
				{
					...
					//					ctItem.t2D描画( CDTXMania.Instance.app.Device, ( x + 0x12 ) * Scale.X, ( y + 12 ) * Scale.Y - 20 );
					//					TextureFactory.tテクスチャの解放( ref ctItem );
					TextureFactory.t安全にDisposeする(ref bmpItem);
				}
				//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 0x12, y + 12, this.list項目リスト[ nItem ].str項目名 );
				//-----------------
				#endregion
				...
				switch (this.list項目リスト[nItem].e種別)
				{
					case CItemBase.E種別.ONorOFFトグル:
						#region [ *** ]
						//-----------------
						//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 210, y + 12, ( (CItemToggle) this.list項目リスト[ nItem ] ).bON ? "ON" : "OFF" );
					...
					case CItemBase.E種別.ONorOFFor不定スリーステート:
						...
						//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 210, y + 12, "ON" );
						break;
					//-----------------
						#endregion
					...
					case CItemBase.E種別.整数:		// #24789 2011.4.8 yyagi: add PlaySpeed supports (copied them from OPTION)
						...
						if (this.list項目リスト[nItem] == this.iCommonPlaySpeed)
						{
							...
							//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 210, y + 12, d.ToString( "0.000" ), ( n行番号 == 0 ) && this.b要素値にフォーカス中 );
						...
						else if (this.list項目リスト[nItem] == this.iDrumsScrollSpeed || this.list項目リスト[nItem] == this.iGuitarScrollSpeed || this.list項目リスト[nItem] == this.iBassScrollSpeed)
						{
							...
							//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 210, y + 12, f.ToString( "x0.0" ), ( n行番号 == 0 ) && this.b要素値にフォーカス中 );
							strParam = f.ToString("x0.0");
						}
						else
						{
							//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 210, y + 12, ( (CItemInteger) this.list項目リスト[ nItem ] ).n現在の値.ToString(), ( n行番号 == 0 ) && this.b要素値にフォーカス中 );
					...
					case CItemBase.E種別.リスト:	// #28195 2012.5.2 yyagi: add Skin supports
						...
						//-----------------
						{
							...
							//CDTXMania.Instance.stageコンフィグ.actFont.t文字列描画( x + 210, y + 12, list.list項目値[ list.n現在選択されている項目番号 ] );
							...
							if (this.list項目リスト[this.n現在の選択項目] == this.iSystemSkinSubfolder)
							{
								...
								if (txSkinSample1 != null)
								{
									txSkinSample1.t2D描画(CDTXMania.Instance.Device, 56 * Scale.X, 300 * Scale.Y);
								}
							}
							#endregion
							break;
				...
				if (b強調)
				{
					...
					txStr.t2D描画(CDTXMania.Instance.Device, (x + 210) * Scale.X, (y + 12) * Scale.Y - 20);
				...
				else
				{
					...
					if (listMenu[nItem].nParam != nIndex || listMenu[nItem].txParam == null)
					{
						...
						Bitmap bmpStr =
							prvFont.DrawPrivateFont(strParam, Color.White, Color.Black);
						stm.txParam = TextureFactory.tテクスチャの生成(bmpStr, false);
						TextureFactory.t安全にDisposeする(ref bmpStr);
			...
			if (this.b項目リスト側にフォーカスがある && (this.n目標のスクロールカウンタ == 0))
			{
				...
				if (this.tx三角矢印 != null)
				{
					this.tx三角矢印.t2D描画(CDTXMania.Instance.Device, x * Scale.X, y_upper * Scale.Y, new Rectangle(0, 0, (int)(32 * Scale.X), (int)(16 * Scale.Y)));
					this.tx三角矢印.t2D描画(CDTXMania.Instance.Device, x * Scale.X, y_lower * Scale.Y, new Rectangle(0, (int)(16 * Scale.Y), (int)(32 * Scale.X), (int)(16 * Scale.Y)));
				}
			}
			//-----------------
			#endregion
		...
		{
			CDTXMania.Instance.ConfigIni.eDark = (Eダークモード)this.iCommonDark.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.n演奏速度 = this.iCommonPlaySpeed.n現在の値;

			CDTXMania.Instance.ConfigIni.bGuitar有効 = (((this.iSystemGRmode.n現在選択されている項目番号 + 1) / 2) == 1);
			//this.iSystemGuitar.bON;
			CDTXMania.Instance.ConfigIni.bDrums有効 = (((this.iSystemGRmode.n現在選択されている項目番号 + 1) % 2) == 1);
			//this.iSystemDrums.bON;

			CDTXMania.Instance.ConfigIni.b全画面モード = this.iSystemFullscreen.bON;
			CDTXMania.Instance.ConfigIni.bSTAGEFAILED有効 = this.iSystemStageFailed.bON;
			CDTXMania.Instance.ConfigIni.bランダムセレクトで子BOXを検索対象とする = this.iSystemRandomFromSubBox.bON;

			CDTXMania.Instance.ConfigIni.bWave再生位置自動調整機能有効 = this.iSystemAdjustWaves.bON;
			CDTXMania.Instance.ConfigIni.b垂直帰線待ちを行う = this.iSystemVSyncWait.bON;
			CDTXMania.Instance.ConfigIni.bバッファ入力を行う = this.iSystemBufferedInput.bON;
			CDTXMania.Instance.ConfigIni.bAVI有効 = this.iSystemAVI.bON;
			CDTXMania.Instance.ConfigIni.bForceAVIFullscreen = this.iSystemForceAVIFullscreen.bON;
			CDTXMania.Instance.ConfigIni.bBGA有効 = this.iSystemBGA.bON;
			//			CDTXMania.Instance.ConfigIni.bGraph有効 = this.iSystemGraph.bON;#24074 2011.01.23 comment-out ikanick オプション(Drums)へ移行
			CDTXMania.Instance.ConfigIni.n曲が選択されてからプレビュー音が鳴るまでのウェイトms = this.iSystemPreviewSoundWait.n現在の値;
			CDTXMania.Instance.ConfigIni.n曲が選択されてからプレビュー画像が表示開始されるまでのウェイトms = this.iSystemPreviewImageWait.n現在の値;
			CDTXMania.Instance.ConfigIni.b演奏情報を表示する = this.iSystemDebugInfo.bON;
			CDTXMania.Instance.ConfigIni.n背景の透過度 = this.iSystemBGAlpha.n現在の値;
			CDTXMania.Instance.ConfigIni.bBGM音を発声する = this.iSystemBGMSound.bON;
			CDTXMania.Instance.ConfigIni.b歓声を発声する = this.iSystemAudienceSound.bON;
			CDTXMania.Instance.ConfigIni.eダメージレベル = (Eダメージレベル)this.iSystemDamageLevel.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.bScoreIniを出力する = this.iSystemSaveScore.bON;

			CDTXMania.Instance.ConfigIni.bログ出力 = this.iLogOutputLog.bON;
			CDTXMania.Instance.ConfigIni.n手動再生音量 = this.iSystemChipVolume.n現在の値;
			CDTXMania.Instance.ConfigIni.n自動再生音量 = this.iSystemAutoChipVolume.n現在の値;
			CDTXMania.Instance.ConfigIni.bストイックモード = this.iSystemStoicMode.bON;

			CDTXMania.Instance.ConfigIni.nShowLagType = this.iSystemShowLag.n現在選択されている項目番号;				// #25370 2011.6.3 yyagi
			CDTXMania.Instance.ConfigIni.bIsAutoResultCapture = this.iSystemAutoResultCapture.bON;					// #25399 2011.6.9 yyagi

			CDTXMania.Instance.ConfigIni.nRisky = this.iSystemRisky.n現在の値;										// #23559 2011.7.27 yyagi

			CDTXMania.Instance.ConfigIni.strSystemSkinSubfolderFullName = skinSubFolders[nSkinIndex];				// #28195 2012.5.2 yyagi
			CDTXMania.Instance.Skin.SetCurrentSkinSubfolderFullName(CDTXMania.Instance.ConfigIni.strSystemSkinSubfolderFullName, true);
			CDTXMania.Instance.ConfigIni.bUseBoxDefSkin = this.iSystemUseBoxDefSkin.bON;								// #28195 2012.5.6 yyagi

			CDTXMania.Instance.ConfigIni.nSoundDeviceType = this.iSystemSoundType.n現在選択されている項目番号;		// #24820 2013.1.3 yyagi
			CDTXMania.Instance.ConfigIni.nWASAPIBufferSizeMs = this.iSystemWASAPIBufferSizeMs.n現在の値;				// #24820 2013.1.15 yyagi
			//			CDTXMania.Instance.ConfigIni.nASIOBufferSizeMs = this.iSystemASIOBufferSizeMs.n現在の値;					// #24820 2013.1.3 yyagi
			CDTXMania.Instance.ConfigIni.nASIODevice = this.iSystemASIODevice.n現在選択されている項目番号;			// #24820 2013.1.17 yyagi
			CDTXMania.Instance.ConfigIni.bUseOSTimer = this.iSystemSoundTimerType.bON;								// #33689 2014.6.17 yyagi

			CDTXMania.Instance.ConfigIni.bTimeStretch = this.iSystemTimeStretch.bON;									// #23664 2013.2.24 yyagi
			//Trace.TraceInformation( "saved" );
			//Trace.TraceInformation( "Skin現在Current : " + CDTXMania.Instance.Skin.GetCurrentSkinSubfolderFullName(true) );
			...
			CDTXMania.Instance.ConfigIni.nMasterVolume = this.iSystemMasterVolume.n現在の値;							// #33700 2014.4.26 yyagi
		...
		{
			//CDTXMania.Instance.ConfigIni.bAutoPlay.Bass = this.iBassAutoPlay.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.BsR = this.iBassR.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.BsG = this.iBassG.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.BsB = this.iBassB.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.BsPick = this.iBassPick.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.BsW = this.iBassW.bON;
			CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Bass = this.iBassScrollSpeed.n現在の値;
			// "Sudden" || "Sud+Hid"
			CDTXMania.Instance.ConfigIni.bSudden.Bass = (this.iBassSudHid.n現在選択されている項目番号 == 1 || this.iBassSudHid.n現在選択されている項目番号 == 3) ? true : false;
			// "Hidden" || "Sud+Hid"
			CDTXMania.Instance.ConfigIni.bHidden.Bass = (this.iBassSudHid.n現在選択されている項目番号 == 2 || this.iBassSudHid.n現在選択されている項目番号 == 3) ? true : false;
			if (this.iBassSudHid.n現在選択されている項目番号 == 4) CDTXMania.Instance.ConfigIni.eInvisible.Bass = EInvisible.SEMI;	// "S-Invisible"
			else if (this.iBassSudHid.n現在選択されている項目番号 == 5) CDTXMania.Instance.ConfigIni.eInvisible.Bass = EInvisible.FULL;	// "F-Invisible"
			else CDTXMania.Instance.ConfigIni.eInvisible.Bass = EInvisible.OFF;
			CDTXMania.Instance.ConfigIni.bReverse.Bass = this.iBassReverse.bON;
			CDTXMania.Instance.ConfigIni.判定文字表示位置.Bass = (E判定文字表示位置)this.iBassPosition.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.eRandom.Bass = (Eランダムモード)this.iBassRandom.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.bLight.Bass = this.iBassLight.bON;
			CDTXMania.Instance.ConfigIni.bLeft.Bass = this.iBassLeft.bON;
			CDTXMania.Instance.ConfigIni.nInputAdjustTimeMs.Bass = this.iBassInputAdjustTimeMs.n現在の値;		// #23580 2011.1.3 yyagi

			CDTXMania.Instance.ConfigIni.b演奏音を強調する.Bass = this.iSystemSoundMonitorBass.bON;
			CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数.Bass = this.iSystemMinComboBass.n現在の値;
			CDTXMania.Instance.ConfigIni.e判定位置.Bass = (E判定位置)this.iSystemJudgePosBass.n現在選択されている項目番号;					// #33891 2014.6.26 yyagi
			//CDTXMania.Instance.ConfigIni.e判定表示優先度.Bass = (E判定表示優先度) this.iBassJudgeDispPriority.n現在選択されている項目番号;
		}
		private void tConfigIniへ記録する_Drums()
		{
			CDTXMania.Instance.ConfigIni.bAutoPlay.LC = this.iDrumsLeftCymbal.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.HH = this.iDrumsHiHat.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.SD = this.iDrumsSnare.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.BD = this.iDrumsBass.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.HT = this.iDrumsHighTom.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.LT = this.iDrumsLowTom.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.FT = this.iDrumsFloorTom.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.CY = this.iDrumsCymbalRide.bON;
			CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Drums = this.iDrumsScrollSpeed.n現在の値;
			CDTXMania.Instance.ConfigIni.ドラムコンボ文字の表示位置 = (Eドラムコンボ文字の表示位置)this.iDrumsComboPosition.n現在選択されている項目番号;
			// "Sudden" || "Sud+Hid"
			CDTXMania.Instance.ConfigIni.bSudden.Drums = (this.iDrumsSudHid.n現在選択されている項目番号 == 1 || this.iDrumsSudHid.n現在選択されている項目番号 == 3) ? true : false;
			// "Hidden" || "Sud+Hid"
			CDTXMania.Instance.ConfigIni.bHidden.Drums = (this.iDrumsSudHid.n現在選択されている項目番号 == 2 || this.iDrumsSudHid.n現在選択されている項目番号 == 3) ? true : false;
			if (this.iDrumsSudHid.n現在選択されている項目番号 == 4) CDTXMania.Instance.ConfigIni.eInvisible.Drums = EInvisible.SEMI;	// "S-Invisible"
			else if (this.iDrumsSudHid.n現在選択されている項目番号 == 5) CDTXMania.Instance.ConfigIni.eInvisible.Drums = EInvisible.FULL;	// "F-Invisible"
			else CDTXMania.Instance.ConfigIni.eInvisible.Drums = EInvisible.OFF;
			CDTXMania.Instance.ConfigIni.bReverse.Drums = this.iDrumsReverse.bON;
			...
			CDTXMania.Instance.ConfigIni.eBDGroup = (EBDGroup)this.iSystemBDGroup.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.eHitSoundPriorityHH = (E打ち分け時の再生の優先順位)this.iSystemHitSoundPriorityHH.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.eHitSoundPriorityFT = (E打ち分け時の再生の優先順位)this.iSystemHitSoundPriorityFT.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.eHitSoundPriorityCY = (E打ち分け時の再生の優先順位)this.iSystemHitSoundPriorityCY.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.bフィルイン有効 = this.iSystemFillIn.bON;
			CDTXMania.Instance.ConfigIni.b演奏音を強調する.Drums = this.iSystemSoundMonitorDrums.bON;
			CDTXMania.Instance.ConfigIni.bドラム打音を発声する = this.iSystemHitSound.bON;
			CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数.Drums = this.iSystemMinComboDrums.n現在の値;
			CDTXMania.Instance.ConfigIni.bシンバルフリー = this.iSystemCymbalFree.bON;

			CDTXMania.Instance.ConfigIni.eDark = (Eダークモード)this.iCommonDark.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.nRisky = this.iSystemRisky.n現在の値;						// #23559 2911.7.27 yyagi
			//CDTXMania.Instance.ConfigIni.e判定表示優先度.Drums = (E判定表示優先度) this.iDrumsJudgeDispPriority.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 = (Eドラムレーン表示位置)this.iDrumsLanePosition.n現在選択されている項目番号;
		}
		private void tConfigIniへ記録する_Guitar()
		{
			//CDTXMania.Instance.ConfigIni.bAutoPlay.Guitar = this.iGuitarAutoPlay.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.GtR = this.iGuitarR.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.GtG = this.iGuitarG.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.GtB = this.iGuitarB.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.GtPick = this.iGuitarPick.bON;
			CDTXMania.Instance.ConfigIni.bAutoPlay.GtW = this.iGuitarW.bON;
			CDTXMania.Instance.ConfigIni.n譜面スクロール速度.Guitar = this.iGuitarScrollSpeed.n現在の値;
			// "Sudden" || "Sud+Hid"
			CDTXMania.Instance.ConfigIni.bSudden.Guitar = (this.iGuitarSudHid.n現在選択されている項目番号 == 1 || this.iGuitarSudHid.n現在選択されている項目番号 == 3) ? true : false;
			// "Hidden" || "Sud+Hid"
			CDTXMania.Instance.ConfigIni.bHidden.Guitar = (this.iGuitarSudHid.n現在選択されている項目番号 == 2 || this.iGuitarSudHid.n現在選択されている項目番号 == 3) ? true : false;
			if (this.iGuitarSudHid.n現在選択されている項目番号 == 4) CDTXMania.Instance.ConfigIni.eInvisible.Guitar = EInvisible.SEMI;	// "S-Invisible"
			else if (this.iGuitarSudHid.n現在選択されている項目番号 == 5) CDTXMania.Instance.ConfigIni.eInvisible.Guitar = EInvisible.FULL;	// "F-Invisible"
			else CDTXMania.Instance.ConfigIni.eInvisible.Guitar = EInvisible.OFF;
			CDTXMania.Instance.ConfigIni.bReverse.Guitar = this.iGuitarReverse.bON;
			CDTXMania.Instance.ConfigIni.判定文字表示位置.Guitar = (E判定文字表示位置)this.iGuitarPosition.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.eRandom.Guitar = (Eランダムモード)this.iGuitarRandom.n現在選択されている項目番号;
			CDTXMania.Instance.ConfigIni.bLight.Guitar = this.iGuitarLight.bON;
			CDTXMania.Instance.ConfigIni.bLeft.Guitar = this.iGuitarLeft.bON;
			CDTXMania.Instance.ConfigIni.nInputAdjustTimeMs.Guitar = this.iGuitarInputAdjustTimeMs.n現在の値;	// #23580 2011.1.3 yyagi

			CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数.Guitar = this.iSystemMinComboGuitar.n現在の値;
			CDTXMania.Instance.ConfigIni.b演奏音を強調する.Guitar = this.iSystemSoundMonitorGuitar.bON;
			CDTXMania.Instance.ConfigIni.e判定位置.Guitar = (E判定位置)this.iSystemJudgePosGuitar.n現在選択されている項目番号;					// #33891 2014.6.26 yyagi
			//CDTXMania.Instance.ConfigIni.e判定表示優先度.Guitar = (E判定表示優先度) this.iGuitarJudgeDispPriority.n現在選択されている項目番号;
		}
		//-----------------
		#endregion
	}
