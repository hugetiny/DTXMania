	...
	internal class CStageタイトル : CStage
	{
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				...
				{
					int x = 0xce;
					int y = 0xcb + ( this.n現在のカーソル行 * 0x27 );
					if( this.ct上移動用.b進行中 )
					{
						y += (int) ( 19.5 * ( Math.Cos( 3.1415926535897931 * ( ( (double) this.ct上移動用.n現在の値 ) / 100.0 ) ) + 1.0 ) );
					}
					else if( this.ct下移動用.b進行中 )
					{
						y -= (int) ( 19.5 * ( Math.Cos( 3.1415926535897931 * ( ( (double) this.ct下移動用.n現在の値 ) / 100.0 ) ) + 1.0 ) );
					}
					if( this.ctカーソルフラッシュ用.n現在の値 <= 100 )
					{
						float num3 = (float) ( 1.0 + ( ( ( (double) this.ctカーソルフラッシュ用.n現在の値 ) / 100.0 ) * 0.5 ) );
						this.txメニュー.vc拡大縮小倍率.X = num3;
						this.txメニュー.vc拡大縮小倍率.Y = num3;
						this.txメニュー.n透明度 = (int) ( 255.0 * ( 1.0 - ( ( (double) this.ctカーソルフラッシュ用.n現在の値 ) / 100.0 ) ) );
						int num4 = x + ( (int) ( ( 227.0 * ( 1.0 - num3 ) ) / 2.0 ) );
						int num5 = y + ( (int) ( ( 39.0 * ( 1.0 - num3 ) ) / 2.0 ) );
						this.txメニュー.t2D描画( CDTXMania.app.Device, num4, num5, new Rectangle( 0, 0xc3, 0xe3, 0x27 ) );
					}
					this.txメニュー.vc拡大縮小倍率.X = 1f;
					this.txメニュー.vc拡大縮小倍率.Y = 1f;
					this.txメニュー.n透明度 = 0xff;
					this.txメニュー.t2D描画( CDTXMania.app.Device, x, y, new Rectangle( 0, 0x9c, 0xe3, 0x27 ) );
				}
				if( this.txメニュー != null )
				{
					this.txメニュー.t2D描画( CDTXMania.app.Device, 0xce, 0xcb, new Rectangle( 0, 0, 0xe3, 0x9c ) );
				}
		...
		private const int MENU_X = 0xce;
		private const int MENU_Y = 0xcb;
