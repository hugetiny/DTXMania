...
namespace DTXMania
{
	internal class CActSelect曲リスト : CActivity
	{
		...
		private List<C曲リストノード> GetSongListWithinMe(C曲リストノード song)
		{
			...
			{
				return CDTXMania.app.Songs管理.list曲ルート;	// rootのリストを返す
			}
			else
			{
		...
		public void t曲リストのソート(DGSortFunc sf, E楽器パート eInst, int order, params object[] p)
		{
			...
			{
				//				CDTXMania.app.Songs管理.t曲リストのソート3_演奏回数の多い順( songList, eInst, order );
				sf(songList, eInst, order, p);
				//				this.r現在選択中の曲 = CDTXMania.app.
		...
		public bool tBOXに入る()
		{
			...
			//Trace.TraceInformation( "Skin現在Current : " + CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName( false ) );
			...
			//Trace.TraceInformation( "Skin現在: " + CSkin.GetSkinName( CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName( false ) ) );
			//Trace.TraceInformation( "Skin現pt: " + CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName( false ) );
			...
			bool ret = false;
			if (CSkin.GetSkinName(CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName(false)) != CSkin.GetSkinName(this.r現在選択中の曲.strSkinPath)
				&& CSkin.bUseBoxDefSkin)
			{
				ret = true;
				// BOXに入るときは、スキン変更発生時のみboxdefスキン設定の更新を行う
				CDTXMania.app.Skin.SetCurrentSkinSubfolderFullName(this.r現在選択中の曲.strSkinPath, false);
			}

			//Trace.TraceInformation( "Skin変更: " + CSkin.GetSkinName( CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName( false ) ) );
			//Trace.TraceInformation( "Skin変更Current : " + CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName( false ) );
		...
		public bool tBOXを出る()
		{
			...
			CDTXMania.app.Skin.SetCurrentSkinSubfolderFullName(
				(this.r現在選択中の曲.strSkinPath == "") ? "" : CDTXMania.app.Skin.GetSkinSubfolderFullNameFromSkinName(CSkin.GetSkinName(this.r現在選択中の曲.strSkinPath)), false);
			//Trace.TraceInformation( "SKIN変更: " + CSkin.GetSkinName( CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName(false) ) );
			//Trace.TraceInformation( "SKIN変更Current : "+  CDTXMania.app.Skin.GetCurrentSkinSubfolderFullName(false) );
			//Trace.TraceInformation( "SKIN変更System  : "+  CSkin.strSystemSkinSubfolderFullName );
			//Trace.TraceInformation( "SKIN変更BoxDef  : "+  CSkin.strBoxDefSkinSubfolderFullName );
		...
		public void t難易度レベルをひとつ進める()
		{
			...
			CDTXMania.app.stage選曲.t選択曲変更通知();
		}
		...
		public void Refresh(CSongs管理 cs, bool bRemakeSongTitleBar)		// #26070 2012.2.28 yyagi
		{
			...
			{
				CDTXMania.app.Songs管理 = cs;

				if (this.r現在選択中の曲 != null)			// r現在選択中の曲==null とは、「最初songlist.dbが無かった or 検索したが1曲もない」
				{
					this.r現在選択中の曲 = searchCurrentBreadcrumbsPosition(CDTXMania.app.Songs管理.list曲ルート, this.r現在選択中の曲.strBreadcrumbs);
					...
					int nMatched = CDTXMania.app.Songs管理.list曲ルート.FindIndex( match );

					this.r現在選択中の曲 = ( nMatched == -1 ) ? null : CDTXMania.app.Songs管理.list曲ルート[ nMatched ];
					this.t現在選択中の曲を元に曲バーを再構成する();
#endif
					return;
				}
		...
		{
			C曲リストノード song = CDTXMania.app.stage選曲.r現在選択中の曲;
			if (song == null)
				return;
			...
			song_last = song;
			List<C曲リストノード> list = (song.r親ノード != null) ? song.r親ノード.list子リスト : CDTXMania.app.Songs管理.list曲ルート;
		...
		public override void On活性化()
		{
			...
			if (CDTXMania.app.ConfigIni.b選曲リストフォントを斜体にする) regular |= FontStyle.Italic;
			if (CDTXMania.app.ConfigIni.b選曲リストフォントを太字にする) regular |= FontStyle.Bold;
			this.ft曲リスト用フォント = new Font(
				CDTXMania.app.ConfigIni.str選曲リストフォント,
				(float)(CDTXMania.app.ConfigIni.n選曲リストフォントのサイズdot * 2 * Scale.Y),		// 後でScale.Yを掛けないように直すこと(Config.ini初期値変更)
				regular,
				GraphicsUnit.Pixel
			);
			...
			if ((this.r現在選択中の曲 == null) && (CDTXMania.app.Songs管理.list曲ルート.Count > 0))
				this.r現在選択中の曲 = CDTXMania.app.Songs管理.list曲ルート[0];
		...
		public override void OnManagedリソースの作成()
		{
			...
			try
			{
				...
				using (Graphics graphics = Graphics.FromImage(image))
				{
					...
					this.txSongNotFound = new CTexture(CDTXMania.app.Device, image, CDTXMania.app.TextureFormat);
			...
			try
			{
				...
				using (Graphics graphics = Graphics.FromImage(image))
				{
					...
					this.txEnumeratingSongs = new CTexture(CDTXMania.app.Device, image, CDTXMania.app.TextureFormat);
		...
		public override int On進行描画()
		{
			...
			{
				for (int i = 0; i < 13; i++)
					this.ct登場アニメ用[i] = new CCounter(-i * 10, 100, 3, CDTXMania.app.Timer);
			...
			else
			{
				...
				while ((n現在時刻 - this.nスクロールタイマ) >= nアニメ間隔)
				{
					...
					if (this.n現在のスクロールカウンタ >= 100)		// １行＝100カウント。
					{
						...
						if (this.n目標のスクロールカウンタ == 0)
							CDTXMania.app.stage選曲.t選択曲変更通知();		// スクロール完了＝選択曲変更！
			...
			switch (color)
			{
				case 0:
					if (this.txスキル数字 != null)
						this.txスキル数字.t2D描画(CDTXMania.app.Device, x, y, new Rectangle((int)(45 * Scale.X) + dx, (int)(24 * Scale.Y) + dy, (int)(9 * Scale.X), (int)(12 * Scale.Y)));
					break;

				case 1:
					if (this.txスキル数字 != null)
						this.txスキル数字.t2D描画(CDTXMania.app.Device, x, y, new Rectangle((int)(45 * Scale.X) + dx, dy, (int)(9 * Scale.X), (int)(12 * Scale.Y)));
					break;

				case 2:
					if (this.txスキル数字 != null)
						this.txスキル数字.t2D描画(CDTXMania.app.Device, x, y, new Rectangle(dx, (int)(24 * Scale.Y) + dy, (int)(9 * Scale.X), (int)(12 * Scale.Y)));
					break;

				case 3:
					if (this.txスキル数字 != null)
						this.txスキル数字.t2D描画(CDTXMania.app.Device, x, y, new Rectangle(dx, dy, (int)(9 * Scale.X), (int)(12 * Scale.Y)));
					break;
			}
		}
		...
		private void t曲名バーの生成(int nバー番号, string str曲名, Color color)
		{
			...
			try
			{
				...
				using (var g = Graphics.FromImage(bmp))
				{
					...
					float y = (((float)bmp.Height) / 2f) - ((CDTXMania.app.ConfigIni.n選曲リストフォントのサイズdot * Scale.Y * 2f) / 2f);
					...
					this.stバー情報[nバー番号].txタイトル名 = new CTexture(CDTXMania.app.Device, bmp, CDTXMania.app.TextureFormat);
		...
		private void tアイテム数の描画_１桁描画(int x, int y, char s数値)
		{
			...
			{
				this.txアイテム数数字.t2D描画(CDTXMania.app.Device, x, y, new Rectangle(dx, dy, (int)(8 * Scale.X), (int)(12 * Scale.Y)));
			}
		}
		//-----------------
		#endregion
