			...
			if( !base.b活性化してない )
			{
				...
				for( int i = -1; i < 4; i++ )
				{
					if( this.txDANGER != null )
					{
						...
						//this.txDANGER.t2D描画( CDTXMania.Instance.app.Device, 0x26 * Scale.X, ( ( ( i * 0x80 ) + num2 ) + 0x40 ) * Scale.Y, this.rc領域[ 1 ] );
						...
						//this.txDANGER.t2D描画( CDTXMania.Instance.app.Device, 0x12a * Scale.X, ( ( ( i * 0x80 ) + num2 ) + 0x40 ) * Scale.Y, this.rc領域[ 1 ] );
					}
				}
			}
			return 0;
