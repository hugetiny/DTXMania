...
//			}
		...
		public override int On進行描画()
		{
			...
			if( !base.b活性化してない )
			{
				...
				#endregion
				if ( ( CDTXMania.Instance.ConfigIni.bSTAGEFAILED有効 && this.actGauge.IsFailed( E楽器パート.DRUMS ) ) && ( base.eフェーズID == CStage.Eフェーズ.共通_通常状態 ) )
				{
...
//			this.actDANGER.t進行描画( this.actGauge.db現在のゲージ値.Drums < 0.3, false, false );
		...
		protected override void t進行描画_Wailing枠()
		{
			...
			base.t進行描画_Wailing枠(
				( CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left ) ?  1761 : 1690,
				( CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left ) ?  1434 : 440,
				yG,
				yB
			//	CDTXMania.Instance.ConfigIni.bReverse.Guitar ? ( 400 - this.txWailing枠.sz画像サイズ.Height ) : 69,	// 350:69
			//	CDTXMania.Instance.ConfigIni.bReverse.Bass ? ( 400 - this.txWailing枠.sz画像サイズ.Height ) : 69		// 350:69
			);
		}
		private void t進行描画_ギターベースフレーム()
		{
			if( ( ( CDTXMania.Instance.ConfigIni.eDark != Eダークモード.HALF ) && ( CDTXMania.Instance.ConfigIni.eDark != Eダークモード.FULL ) ) && CDTXMania.Instance.ConfigIni.bGuitar有効 )
			{
		...
		protected override void t進行描画_パネル文字列()
		{
			...
			Point p = pd[ (int) CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 ];
					...
					switch( ( (Eパッド) nPad ) )
					{
						...
						case Eパッド.CY:
							...
							//-----------------------------
							{
								...
								CChip chipLC = CDTXMania.Instance.ConfigIni.bシンバルフリー ? this.r指定時刻に一番近い未ヒットChip( nTime, Ech定義.LeftCymbal, nInputAdjustTime ) : null;
						...
						case Eパッド.LC:
							...
							//-----------------------------
							{
								...
								CChip chipLC = this.r指定時刻に一番近い未ヒットChip( nTime, Ech定義.LeftCymbal, nInputAdjustTime );	// LC
								CChip chipCY = CDTXMania.Instance.ConfigIni.bシンバルフリー ? this.r指定時刻に一番近い未ヒットChip( nTime, Ech定義.Cymbal, nInputAdjustTime ) : null;
								CChip chipRD = CDTXMania.Instance.ConfigIni.bシンバルフリー ? this.r指定時刻に一番近い未ヒットChip( nTime, Ech定義.RideCymbal, nInputAdjustTime ) : null;
								E判定 e判定HC = ( chipHC != null ) ? this.e指定時刻からChipのJUDGEを返す( nTime, chipHC, nInputAdjustTime ) : E判定.Miss;
								E判定 e判定HO = ( chipHO != null ) ? this.e指定時刻からChipのJUDGEを返す( nTime, chipHO, nInputAdjustTime ) : E判定.Miss;
								E判定 e判定LC = ( chipLC != null ) ? this.e指定時刻からChipのJUDGEを返す( nTime, chipLC, nInputAdjustTime ) : E判定.Miss;
								E判定 e判定CY = ( chipCY != null ) ? this.e指定時刻からChipのJUDGEを返す( nTime, chipCY, nInputAdjustTime ) : E判定.Miss;
		...
		protected override void t進行描画・チップ・ギターベース( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, E楽器パート inst )
		{
			...
			if ( configIni.bGuitar有効 )
			{
					...
					if ( ( y > 57 ) && ( y < 412 ) )
					{
						if ( this.txチップ != null )
						{
							...
							if ( pChip.nチャンネル番号 == OPEN )
							{
								...
								this.txチップ.t2D描画( CDTXMania.Instance.app.Device, xo, y - 2, new Rectangle( 268, 144 + ( ( nアニメカウンタ現在の値 % 5 ) * 6 ), 76, 6 ) );
							}
							...
							if ( bChipHasR )
							{
								this.txチップ.t2D描画( CDTXMania.Instance.app.Device, x, y - 3, rc );
							}
							rc.X += 24;
							x += deltaX;
							if ( bChipHasG )
				...
				if ( ( !pChip.bHit ) && ( pChip.nバーからの距離dot[ instIndex ] < 0 ) )
				{
					...
					if ( (inst == E楽器パート.GUITAR && bIsAutoPlay.GtPick ) || (inst == E楽器パート.BASS && bIsAutoPlay.BsPick ) )
					{
						...
						pChip.bHit = true;
						this.tサウンド再生( pChip, CDTXMania.Instance.Timer.n前回リセットした時のシステム時刻 + pChip.n発声時刻ms, inst, dTX.nモニタを考慮した音量( inst ), false, bMiss );
			...
			{
				pChip.bHit = true;
				this.tサウンド再生( pChip, CDTXMania.Instance.Timer.n前回リセットした時のシステム時刻 + pChip.n発声時刻ms, inst, dTX.nモニタを考慮した音量( inst ) );
			}
		}
#endif
		protected void t進行描画_チップ_ギター_ウェイリング( CConfigIni configIni, ref CDTX dTX, ref CChip pChip )
		{
			...
			//            {
			//                this.txチップ.t2D描画( CDTXMania.Instance.app.Device,
			//                    drawX,
			//                    ( ( y - numA ) + numC ),
			//                    rect
			//                );
		...
		protected override void t進行描画・チップ・ベース( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			if ( configIni.bGuitar有効 )
			{
				...
				if ( !pChip.bHit && pChip.b可視 )
				{
					...
					if ( ( y > 57 ) && ( y < 412 ) )
					{
						...
						if ( pChip.nチャンネル番号 == 0xA0 )
						{
							if ( this.txチップ != null )
							{
								this.txチップ.t2D描画( CDTXMania.Instance.app.Device, 400, y - 2, new Rectangle( 268, 144 + ( ( nアニメカウンタ現在の値 % 5 ) * 6 ), 76, 6 ) );
							}
						}
						else if ( this.txチップ != null )
						{
							...
							if ( bGtBsR )
							{
								this.txチップ.t2D描画( CDTXMania.Instance.app.Device, 400, y - 3, rc );
							}
							rc.X += 24;
							x += deltaX;
							if ( bGtBsG )
							{
								this.txチップ.t2D描画( CDTXMania.Instance.app.Device, 0x1aa, y - 3, rc );
							}
							rc.X += 24;
							x += deltaX;
							if ( bGtBsB )
							{
								this.txチップ.t2D描画( CDTXMania.Instance.app.Device, 0x1c4, y - 3, rc );
							}
						}
					}
				}
				if ( ( configIni.bAutoPlay.Bass && !pChip.bHit ) && ( pChip.nバーからの距離dot.Bass < 0 ) )
				{
					...
					{
						pChip.bHit = true;
						this.tサウンド再生( pChip, CDTXMania.Instance.Timer.n前回リセットした時のシステム時刻 + pChip.n発声時刻ms, E楽器パート.BASS, dTX.nモニタを考慮した音量( E楽器パート.BASS ) );
...
//#endif
			...
			if ( ( pChip.b可視 && configIni.bGuitar有効 ) && ( configIni.eDark != Eダークモード.FULL ) && ( this.txチップ != null ) )
			{
					...
					)
				{
					this.txチップ.t2D描画(
						CDTXMania.Instance.Device,
						(CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left)? 1521 : 1450,
						y,
						new Rectangle(
							0,
							(int) ( 450 * Scale.Y ),
					...
					)
				{
					this.txチップ.t2D描画(
						CDTXMania.Instance.Device,
						( CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left ) ? 1194 : 200,
						y,
						new Rectangle(
							0,
							(int) ( 450 * Scale.Y ),
