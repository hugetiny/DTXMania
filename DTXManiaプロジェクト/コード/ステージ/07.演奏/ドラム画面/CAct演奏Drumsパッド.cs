...
namespace DTXMania
{
	internal class CAct演奏Drumsパッド : CActivity
	{
		...
		private static readonly ST基本位置[] st基本位置 = new ST基本位置[9]
		{
			...
			if (!base.b活性化してない)
			{
				if (!CDTXMania.Instance.ConfigIni.bギタレボモード)
				{
					...
					for (int i = 0; i < 9; i++)
					{
						...
						x += xDist;
						int y = (st基本位置[index].y + (CDTXMania.Instance.ConfigIni.bReverse.Drums ? -10 : 0x19e)) + this.stパッド状態[index].nY座標オフセットdot;
						if (this.txパッド != null)
						{
							this.txパッド.t2D描画(
