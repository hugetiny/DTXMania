...
namespace DTXMania
{
	...
	public class CWailingChip共通
	{
		...
		protected const int WailingHeight = (int)120;	// ウェイリングチップ画像の高さ: 4種全て同じ値
		...
		internal void t進行描画_チップ_ウェイリング(ref CChip pChip, ref CTexture txチップ,
			ref C演奏判定ライン座標共通 演奏判定ライン座標, ref CCounter ctWailingチップ模様アニメ)
		{
			...
			if (CDTXMania.Instance.ConfigIni.bGuitar有効)
			{
				if (!pChip.bHit && pChip.b可視)
				{
					...
					int y = CDTXMania.Instance.ConfigIni.bReverse[(int)gb] ?
						 (y_base[1] - (int)(pChip.nバーからの距離dot[(int)gb])) :
						 (y_base[0] + (int)(pChip.nバーからの距離dot[(int)gb]));
	...
	{
		internal CWailngChip_Guitar_Drum画面(ref C演奏判定ライン座標共通 演奏判定ライン座標)
			: base()
		{
			eInst = E楽器パート.GUITAR;

			base.y_base[0] = 演奏判定ライン座標.n判定ラインY座標(eInst, false);		// 95
			base.y_base[1] = 演奏判定ライン座標.n判定ラインY座標(eInst, true);		// 374
			...
			drawX = (CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left) ? 1764 : 1764 - 71;	// ウェイリングチップ描画位置X座標: 4種全て異なる値
	...
	{
		internal CWailngChip_Bass_Drum画面(ref C演奏判定ライン座標共通 演奏判定ライン座標)
			: base()
		{
			eInst = E楽器パート.BASS;
			...
			drawX = (CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left) ? 1437 : 1437 - 994;	// ウェイリングチップ描画位置X座標: 4種全て異なる値
	...
	{
		internal CWailngChip_Guitar_GR画面(ref C演奏判定ライン座標共通 演奏判定ライン座標)
			: base()
		{
			eInst = E楽器パート.GUITAR;
			base.y_base[0] = 演奏判定ライン座標.n判定ラインY座標(eInst, false);	// 95
	...
	{
		internal CWailngChip_Bass_GR画面(ref C演奏判定ライン座標共通 演奏判定ライン座標)
			: base()
		{
			eInst = E楽器パート.BASS;
			base.y_base[0] = 演奏判定ライン座標.n判定ラインY座標(eInst, false);	// 95
