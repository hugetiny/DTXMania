...
namespace DTXMania
{
	internal class CAct演奏Combo共通 : CActivity
	{
		...
		protected virtual void tコンボ表示_ドラム( int nCombo値, int nジャンプインデックス )
		{
			#region [ 事前チェック。]
			//-----------------
			if( CDTXMania.Instance.ConfigIni.ドラムコンボ文字の表示位置 == Eドラムコンボ文字の表示位置.OFF )
				return;		// 表示OFF。
			...
			bool guitar = CDTXMania.Instance.DTX.bチップがある.Guitar;
			bool bass = CDTXMania.Instance.DTX.bチップがある.Bass;
			var e表示位置 = CDTXMania.Instance.ConfigIni.ドラムコンボ文字の表示位置;
			...
			switch( e表示位置 )
			{
				case Eドラムコンボ文字の表示位置.LEFT:
					nX中央位置px = ( CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left ) ?(int) ( 187 * Scale.X ) : 360;
					break;

				case Eドラムコンボ文字の表示位置.CENTER:
					nX中央位置px = ( CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left ) ? (int) ( 320 * Scale.X ) : 960;
					break;

				case Eドラムコンボ文字の表示位置.RIGHT:
					nX中央位置px = ( CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left ) ?(int) ( 485 * Scale.X ) : 1920 - 340;
					break;
			}
			int nY上辺位置px = CDTXMania.Instance.ConfigIni.bReverse.Drums ? (int) ( 350 * Scale.Y ) : (int) ( 60 * Scale.Y );
			...
			for( int i = 0; i < n桁数; i++ )
			{
				...
				if( this.txCOMBOドラム != null )
				{
					this.txCOMBOドラム.t2D描画( CDTXMania.Instance.Device,
						x, y,
						new Rectangle(
							( n位の数[ i ] % 4 ) * nドラムコンボの幅,
							( n位の数[ i ] / 4 ) * nドラムコンボの高さ,
		...
		public override int On進行描画()
		{
			...
			for( int i = 2; i >= 0; i-- )
			{
				...
				else if( ( this.status[ i ].n現在表示中のCOMBO値 < CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数[ i ] ) && ( this.status[ i ].nCOMBO値 < CDTXMania.Instance.ConfigIni.n表示可能な最小コンボ数[ i ] ) )
				{
				...
				{
					if( ( this.status[ i ].n前回の時刻_ジャンプ用 == -1 ) || ( CDTXMania.Instance.Timer.n現在時刻 < this.status[ i ].n前回の時刻_ジャンプ用) )
						this.status[ i ].n前回の時刻_ジャンプ用 = CDTXMania.Instance.Timer.n現在時刻;

					const long INTERVAL = 2;
					while( ( CDTXMania.Instance.Timer.n現在時刻 - this.status[ i ].n前回の時刻_ジャンプ用) >= INTERVAL )
					{
				...
				switch( this.status[ i ].e現在のモード )
				{
					...
					case EMode.進行表示中:
						...
						if( ( e今回の状態遷移イベント == EEvent.非表示 ) || ( e今回の状態遷移イベント == EEvent.ミス通知 ) )
						{
							...
							this.status[ i ].nコンボが切れた時刻 = CDTXMania.Instance.Timer.n現在時刻;
							goto Retry;
						}
					...
					case EMode.残像表示中:
						...
						if( ( CDTXMania.Instance.Timer.n現在時刻 - this.status[ i ].nコンボが切れた時刻 ) > 1000 )
						{
							// モード変更２
							this.status[ i ].e現在のモード = EMode.非表示中;
							goto Retry;
