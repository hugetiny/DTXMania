	...
	internal class CStage演奏ギター画面 : CStage演奏画面共通
	{
		...
		public override int On進行描画()
		{
			if (!base.b活性化してない)
			{
				...
				if (CDTXMania.app.ConfigIni.bSTAGEFAILED有効 && (base.eフェーズID == CStage.Eフェーズ.共通_通常状態))
				{
					//					bool flag3 = ( CDTXMania.app.ConfigIni.bAutoPlay.Guitar || !CDTXMania.app.DTX.bチップがある.Guitar ) || ( this.actGauge.db現在のゲージ値.Guitar <= -0.1 );				// #23630
					//					bool flag4 = ( CDTXMania.app.ConfigIni.bAutoPlay.Bass || !CDTXMania.app.DTX.bチップがある.Bass ) || ( this.actGauge.db現在のゲージ値.Bass <= -0.1 );					// #23630
					...
					bool bFailedNoChips = (!CDTXMania.app.DTX.bチップがある.Guitar && !CDTXMania.app.DTX.bチップがある.Bass);	// #25216 2011.5.21 yyagi add condition
		...
		protected override void t進行描画_Wailing枠()
		{
			...
			base.t進行描画_Wailing枠(
				...
				yG,
				yB
				//CDTXMania.app.ConfigIni.bReverse.Guitar ? 340 : 11,
				//CDTXMania.app.ConfigIni.bReverse.Bass ?   340 : 11
			);
		}
		...
		protected override void t背景テクスチャの生成()
		{
			...
			if ((CDTXMania.app.DTX.BACKGROUND_GR != null) && (CDTXMania.app.DTX.BACKGROUND_GR.Length > 0))
			{
				BACKGROUND = CDTXMania.app.DTX.BACKGROUND_GR;
			}
			else if ((CDTXMania.app.DTX.BACKGROUND != null) && (CDTXMania.app.DTX.BACKGROUND.Length > 0))
			{
				BACKGROUND = CDTXMania.app.DTX.BACKGROUND;
			}
			if ((BACKGROUND != null) && (BACKGROUND.Length > 0))
			{
				BgFilename = CDTXMania.app.DTX.strフォルダ名 + BACKGROUND;
			}
		...
		protected override void t進行描画_チップ_フィルイン(CConfigIni configIni, ref CDTX dTX, ref CChip pChip)
		{
			...
			switch ( pChip.n整数値 )
			{
				...
				case 0x06:	// ギター消音あり(従来同等)
					CDTXMania.app.DTX.b演奏で直前の音を消音する.Guitar = true;
					break;
				case 0x07:	// ギター消音無し
					CDTXMania.app.DTX.b演奏で直前の音を消音する.Guitar = false;
					break;
