			...
			if (base.b活性化してる)
			{
				...
				if ((this.strパネル文字列 != null) && (this.strパネル文字列.Length > 0))
				{
					...
					try
					{
						...
						graphics.Dispose();
						this.txPanel = new CTexture(CDTXMania.Instance.Device, bitmap2, CDTXMania.Instance.TextureFormat);
					...
					this.ct進行用 = new CCounter((int)(-278 * Scale.X), this.n文字列の長さdot / 2, unchecked((int)(12.0f / Scale.X)), CDTXMania.Instance.Timer);
				}
				this.Start();
			}
		}
		...
		public int t進行描画(int x, int y)
		{
			if (!base.b活性化してない && !this.bMute)
			{
				...
				Rectangle rectangle = new Rectangle(
					...
					0,
					(CDTXMania.Instance.ConfigIni.eドラムレーン表示位置 == Eドラムレーン表示位置.Left) ?
				...
				this.txPanel.t2D描画(CDTXMania.Instance.Device, x * Scale.X, y * Scale.Y, rectangle);
			}
			return 0;
		}
