...
namespace DTXMania
{
	...
	internal class CSongs管理
	{
					...
					if (File.Exists(infoDir.FullName + @"\box.def"))
					{
						...
						if (boxdef.SkinPath != null)
						{
							...
							else
							{
								...
								if (CDTXMania.Instance.Skin.bIsValid(s))
								{
		...
		public void t曲リストへ後処理を適用する()
		{
			...
			if (CDTXMania.Instance.Skin.strBoxDefSkinSubfolders != null)
			{
				foreach (string b in CDTXMania.Instance.Skin.strBoxDefSkinSubfolders)
				{
			...
			string[] systemSkinNames = CSkin.GetSkinName(CDTXMania.Instance.Skin.strSystemSkinSubfolders);
			...
			Array.Sort(ba);
			CDTXMania.Instance.Skin.strBoxDefSkinSubfolders = ba;
		}
