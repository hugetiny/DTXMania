								...
								else
								{
									...
									if( node.arスコア[ lv ].ScoreIni情報.最終更新日時 != this.listSongsDB[ nMatched ].ScoreIni情報.最終更新日時 )
									{
										...
										try
										{
											...
											for( int i = 0; i < 3; i++ )
											{
												int nSectionHiSkill = ( i * 3 ) + 1;		// i*2 -> i*3
												...
												node.arスコア[ lv ].譜面情報.最大演奏型スキル[ i ] = scoreIni.stセクション[ nSectionHiSkill ].db演奏型スキル値;
												node.arスコア[ lv ].譜面情報.最大ゲーム型スキル[ i ] = scoreIni.stセクション[ nSectionHiSkill ].dbゲーム型スキル値;	// #23624 2011.5.10 yyagi
											...
											for( int i = 0; i < 5; i++ )
											{
												node.arスコア[ lv ].譜面情報.演奏履歴[ i ] =  scoreIni.stファイル.History[ i ];
											}
											if( CDTXMania.ConfigIni.bLog曲検索ログ出力 )
											{
...
//Debug.WriteLine( "★" + this.nファイルから反映できたスコア数 + " " + c曲リストノード.arスコア[ i ].譜面情報.タイトル );
							...
							string strScoreIniファイルパス = c曲リストノード.arスコア[ i ].ファイル情報.ファイルの絶対パス + ".score.ini";
							if( File.Exists( strScoreIniファイルパス ) )
							{
								try
								{
									CScoreIni scoreIni = new CScoreIni( strScoreIniファイルパス );
									scoreIni.t全演奏記録セクションの整合性をチェックし不整合があればリセットする();
									for( int j = 0; j < 3; j++ )
									{
										int nSectionHiSkill = ( j * 3 ) + 1;
										if( ( scoreIni.stセクション[ nSectionHiSkill ].b演奏にMIDI入力を使用した || scoreIni.stセクション[ nSectionHiSkill ].b演奏にキーボードを使用した ) || ( scoreIni.stセクション[ nSectionHiSkill ].b演奏にジョイパッドを使用した || scoreIni.stセクション[ nSectionHiSkill ].b演奏にマウスを使用した ) )
										{
											c曲リストノード.arスコア[ i ].譜面情報.最大ランク[ j ] = CScoreIni.tランク値を計算して返す( scoreIni.stセクション[ nSectionHiSkill ].n全チップ数, scoreIni.stセクション[ nSectionHiSkill ].nPerfect数, scoreIni.stセクション[ nSectionHiSkill ].nGreat数, scoreIni.stセクション[ nSectionHiSkill ].nGood数, scoreIni.stセクション[ nSectionHiSkill ].nPoor数, scoreIni.stセクション[ nSectionHiSkill ].nMiss数 );
										}
										else
										{
											c曲リストノード.arスコア[ i ].譜面情報.最大ランク[ j ] = (int) CScoreIni.ERANK.UNKNOWN;	// 0x63;
										}
										c曲リストノード.arスコア[ i ].譜面情報.最大演奏型スキル[ j ] = scoreIni.stセクション[ nSectionHiSkill ].db演奏型スキル値;
										c曲リストノード.arスコア[i].譜面情報.最大ゲーム型スキル[j] =	// #23624 2011.5.10 yyagi
											scoreIni.stセクション[nSectionHiSkill + 1].bExist ?						// まだ[HiGameSkill.*]が作られていなければ、ゲーム方スキル値をイチから計算する
											scoreIni.stセクション[nSectionHiSkill].dbゲーム型スキル値 : 
											CScoreIni.tゲーム型スキルを計算して返す(
												scoreIni.stセクション[nSectionHiSkill].n全チップ数,
												0,
												scoreIni.stセクション[nSectionHiSkill].nPerfect数,
												scoreIni.stセクション[nSectionHiSkill].nGreat数,
												scoreIni.stセクション[nSectionHiSkill].nGood数,
												scoreIni.stセクション[nSectionHiSkill].nPoor数,
												scoreIni.stセクション[nSectionHiSkill].nMiss数,
												scoreIni.stセクション[nSectionHiSkill].n最大コンボ数
											);
										c曲リストノード.arスコア[i].譜面情報.フルコンボ[j] = scoreIni.stセクション[nSectionHiSkill].bフルコンボである;
									}
									c曲リストノード.arスコア[ i ].譜面情報.演奏回数.Drums = scoreIni.stファイル.PlayCountDrums;
									c曲リストノード.arスコア[ i ].譜面情報.演奏回数.Guitar = scoreIni.stファイル.PlayCountGuitar;
									c曲リストノード.arスコア[ i ].譜面情報.演奏回数.Bass = scoreIni.stファイル.PlayCountBass;
									for( int j = 0; j < 5; j++ )
									{
										c曲リストノード.arスコア[ i ].譜面情報.演奏履歴[ j ] = scoreIni.stファイル.History[ j ];
									}
								}
								catch
								{
		...
		public void tScoreIniを読み込んで譜面情報を設定する( string strScoreIniファイルパス, ref Cスコア score )
		{
			...
			try
			{
				...
				for( int n楽器番号 = 0; n楽器番号 < 3; n楽器番号++ )
				{
					...
					//-----------------
					#endregion
					score.譜面情報.最大演奏型スキル[ n楽器番号 ] = ini.stセクション[ n ].db演奏型スキル値;
					score.譜面情報.最大ゲーム型スキル[ n楽器番号 ] = ini.stセクション[ n ].dbゲーム型スキル値;
