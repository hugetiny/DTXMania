...
namespace DTXMania
{
	internal class CActFIFOWhite : CActivity
	{
		...
		public override int On進行描画()
		{
			...
			Size clientSize = CDTXMania.app.Window.ClientSize;
			if( this.tx白タイル64x64 != null )
			{
				...
				for( int i = 0; i <= ( clientSize.Width / 0x40 ); i++ )
				{
					for( int j = 0; j <= ( clientSize.Height / 0x40 ); j++ )
					{
						this.tx白タイル64x64.t2D描画( CDTXMania.app.Device, i * 0x40, j * 0x40 );
					}
				}
			}
