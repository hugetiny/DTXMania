...
namespace DTXMania
{
	internal class CActResultImage : CActivity
	{
		...
		{
			int num = CScoreIni.t総合ランク値を計算して返す( CDTXMania.stage結果.st演奏記録.Drums, CDTXMania.stage結果.st演奏記録.Guitar, CDTXMania.stage結果.st演奏記録.Bass );
			if( string.IsNullOrEmpty( CDTXMania.DTX.RESULTIMAGE[ num ] ) )
			{
				return false;
			}
			CDTXMania.tテクスチャの解放( ref this.txリザルト画像 );
			this.r表示するリザルト画像 = null;
			string path = CDTXMania.DTX.strフォルダ名 + CDTXMania.DTX.RESULTIMAGE[ num ];
			if( !File.Exists( path ) )
			{
				Trace.TraceWarning( "ファイルが存在しません。({0})", new object[] { path } );
				return false;
		...
		private bool tリザルト動画の指定があれば構築する()
		{
			...
			int num = CScoreIni.t総合ランク値を計算して返す( CDTXMania.stage結果.st演奏記録.Drums, CDTXMania.stage結果.st演奏記録.Guitar, CDTXMania.stage結果.st演奏記録.Bass );
			if( string.IsNullOrEmpty( CDTXMania.DTX.RESULTMOVIE[ num ] ) )
			{
				return false;
			}
			this.strAVIファイル名 = CDTXMania.DTX.strフォルダ名 + CDTXMania.DTX.RESULTMOVIE[ num ];
			if( !File.Exists( this.strAVIファイル名 ) )
			{
				Trace.TraceWarning( "ファイルが存在しません。({0})", new object[] { this.strAVIファイル名 } );
				return false;
